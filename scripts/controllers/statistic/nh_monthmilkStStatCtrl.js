(function() {
    ...
    angular
        .module('newhope')
        .controller('MonthmilkStStatCtrl', MonthmilkStStatCtrl);

    MonthmilkStStatCtrl.$inject = ['$window','$rootScope', '$scope', '$state', '$alert', '$uibModal', 'restService'];

    function MonthmilkStStatCtrl($window, $rootScope, $scope, $state, $alert, $uibModal, rest) {
        ...
        vm.getData = function(pageno) {
            ...
            rest.findMonthReport(params).then(function(json) {
                vm.tbLoding = 0;
        ...
        $scope.monthReportOutput = function(){
            var searchParams={
                theDate:moment($scope.search.fromDate).format('YYYY-MM-DD'),
                branchNo:$scope.search.branchNo,
                dealerId:$scope.search.dealerNo
            }
            rest.findMonthReportOutput(searchParams).then(function(json){
