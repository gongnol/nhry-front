(function() {
	...
	angular
	  .module('newhope')
	  .controller('currentYearOrderCtrl', currentYearOrderCtrl)  
	...
	currentYearOrderCtrl.$inject = ['$rootScope','$state','$stateParams','$scope','$uibModal', '$alert','restService', 'nhCommonUtil'];  

	function currentYearOrderCtrl($rootScope, $state, $stateParams, $scope, $uibModal,  $alert , rest, nhCommonUtil) {
        ...
        vm.getData = function(pageno){ 
            ...
            if (vm.batchMps && vm.batchMps.length > 0) {
                ...
                params = {
                    ...
                    promSubType: 'Z017',
                    preorderStat20F70Z017:'20'//查询出年卡未确认订单
                }
        ...
        vm.toOrderEdit = function(orderNo){
            var url = $state.href('newhope.orderEdit', {orderNo: orderNo,isYearcard : true});
            window.open(url,'_blank');
        };
    ...
    function CancelOrderModal($scope, $uibModalInstance, $alert,orderNo,rest) {
        ...
        function save() {
            ...
            if (!vm.isStopAhead) {
                ...
                rest.yearCardBackOrder(vm.handle).then(function(json){
                ...
                vm.cancelOrderSaving = true;
                rest.advanceYearCardBackOrder(vm.handle).then(function(json){
                    var result = json.type;
                    if(result == 'success'){
                        var saveAlert = $alert({
