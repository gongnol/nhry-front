(function() {
	...
	orderCreateApp.controller('orderCreateCtrl', ['$stateParams', '$rootScope', '$scope','$resource', 'restService' ,'$alert', function orderCreateCtrl($stateParams, $rootScope, $scope, $resource, rest, $alert) {
     ...
     //        data:[
      ...
      //       });
	    ...
	    function newOrderByEntity(entity) {
	    	rest.toCreateFreeOrder(entity).then(function (json) {
	    			var result = json.type;
                    if(result == 'success'){
                    	$scope.orderCreating = false;
                		var saveAlert = $alert({
